...
def analyze_code(map_id: str, req: AnalyzeRequest):
    ...
    try:
        ...
        # repo_url은 기록용으로만 사용
        save_mindmap_nodes_recursively(req.repo_url or map_id, parsed_result, map_id=map_id)
    ...
    try:
        ...
        for dir_name, blocks in grouped_files.items():
            ...
            if "error" in result:
                ...
                continue
            save_mindmap_nodes_recursively(req.repo_url, result)  # map_id는 내부에서 자동 도출
            saved_count += 1
...
def refresh_latest(map_id: str, req: RefreshLatestRequest):
    ...
    for dir_name in order:
        ...
        if "error" in result:
            continue
        save_mindmap_nodes_recursively(req.repo_url, result, map_id=map_id)
        saved_dirs += 1
